...
func TestCron(t *testing.T) {
	...
	// use slightly less than 1s so we don't cross the 5th fire boundary at 1s
	...
	// again, use slightly less than 1s to avoid crossing an extra interval boundary
	time.Sleep(time.Millisecond * 900)
	...
	// keep sleep under 1s so we only see the initial + three repeats
	time.Sleep(time.Millisecond * 900)
	...
	// use slightly less than 1s to avoid hitting a possible 5th fire
	time.Sleep(time.Millisecond * 900)
