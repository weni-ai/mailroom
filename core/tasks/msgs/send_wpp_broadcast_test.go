...
func TestWppBroadcastTask(t *testing.T) {
	...
	tcs := []struct {
		...
		},
		{
			...
			cathyOnly,
			doctorsOnly,
			queue.TemplateNotificationBatchQueue,
			2,
			121,
			baseMsg,
			"hello world",
	...
	for i, tc := range tcs {
		...
		bcast := models.NewWppBroadcast(oa.OrgID(), tc.BroadcastID, tc.Msg, tc.URNs, tc.ContactIDs, tc.GroupIDs, tc.ChannelID, tc.Queue)
