...
func TestClearQueuedOrPending(t *testing.T) {
	ctx, rt, db, rp := testsuite.Get()
	rc := rp.Get()
	defer rc.Close()
	...
	// should have one queued connection
	client.callError = nil
	client.callID = ivr.CallID("call1")
	err = HandleFlowStartBatch(ctx, rt, batch)
	assert.NoError(t, err)
	...
	err = clearQueuedAndPendingConnections(ctx, rt, "clear_queued_test", "clear_queued_test")

	// should have 0 queued connections
	testsuite.AssertQuery(t, db, `SELECT COUNT(*) FROM channels_channelconnection WHERE contact_id = $1 AND status = $2`,
		testdata.Cathy.ID, models.ConnectionStatusQueued).Returns(0)
	...
	testsuite.AssertQuery(t, db, `SELECT COUNT(*) FROM channels_channelconnection WHERE contact_id = $1 AND status = $2`,
		testdata.Cathy.ID, models.ConnectionStatusWired).Returns(0)
}
