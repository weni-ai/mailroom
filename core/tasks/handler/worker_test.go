...
func TestRequestToRouter(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:            "stream support enabled, webchat preview disabled",
			channelConfig:   map[string]interface{}{"version": 2},
			expectedStream:  true,
			expectedPreview: false,
		},
		{
			name:            "stream support enabled, webchat preview enabled",
			channelConfig:   map[string]interface{}{"version": 2, "preview": true},
			expectedStream:  true,
			expectedPreview: true,
		},
		{
			name:            "stream support disabled, webchat preview disabled",
			channelConfig:   map[string]interface{}{},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			channel := testdata.InsertChannel(db, testdata.Org1, "TW", "Router Channel", []string{"tel"}, "SR", tt.channelConfig)
			...
			assert.Equal(t, tt.expectedStream, payload.StreamSupport)
			assert.Equal(t, tt.expectedPreview, payload.Preview)
...
func TestRequestToRouterError(t *testing.T) {
	ctx, rt, db, _ := testsuite.Get()
	defer testsuite.Reset(testsuite.ResetAll)

	channel := testdata.InsertChannel(db, testdata.Org1, "TW", "Error Channel", []string{"tel"}, "SR", map[string]interface{}{})
	contact := testdata.InsertContact(db, testdata.Org1, flows.ContactUUID(uuids.New()), "Error Contact", envs.Language("eng"))
	urn := urns.URN("tel:+250700000099")
	urnID := testdata.InsertContactURN(db, testdata.Org1, contact, urn, 2000)
	...
	tests := []struct {
	...
	}{
		{
			...
			expectedErrMsg: "router call error: status code 400",
		},
		{
			name:           "internal server error returns error",
			statusCode:     http.StatusInternalServerError,
			expectedErrMsg: "router call error: status code 500",
		},
	}
