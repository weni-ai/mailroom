...
func TestTemplates(t *testing.T) {
	...
	assert.Equal(t, "welcome", templates[2].Name())
	...
	assert.Equal(t, 1, len(templates[2].Translations()))
	tt = templates[2].Translations()[0]
	assert.Equal(t, envs.Language("eng"), tt.Language())
	assert.Equal(t, envs.Country("US"), tt.Country())
	assert.Equal(t, "7300ee93-b610-4ea5-98f0-f49d66dba123", tt.Namespace())
	assert.Equal(t, testdata.WhatsAppCloudChannel.UUID, tt.Channel().UUID)
	assert.Equal(t, "Welcome {{1}}!", tt.Content())
}
